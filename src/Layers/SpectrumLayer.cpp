#include "include/Layers/SpectrumLayer.h"
...
SpectrumLayer::SpectrumLayer() : m_Container(ThemeUtils::GetRightContainerParams())
{
...
void SpectrumLayer::createRandomCurves(SkRect bounds, int numCurves) {
...
void SpectrumLayer::updateCurves(SkRect bounds) {
...
void SpectrumLayer::onPaint(SkSurface* surface)
{
...
bool SpectrumLayer::onKey(skui::Key key, skui::InputState state, skui::ModifierKey modifier)
{
	return m_Container.ProcessKey(key, state, modifier);
}

bool SpectrumLayer::onMouse(int x, int y, skui::InputState state, skui::ModifierKey modifier)
{
